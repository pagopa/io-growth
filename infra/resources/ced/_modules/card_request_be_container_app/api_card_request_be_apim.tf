resource "azurerm_api_management_product" "ced-card-be" {
...
resource "azurerm_api_management_product_policy" "ced-card-be" {
  product_id          = azurerm_api_management_product.ced-card-be.product_id
...
resource "azurerm_api_management_api" "ced_card_be_v1" {
  name = format("%s-%s-card-be-public-api", var.environment.prefix, var.environment.env_short)
  ...
  version_set_id = azurerm_api_management_api_version_set.ced-card-be.id
  version        = "v1"
  revision       = "1"
...
resource "azurerm_api_management_named_value" "ced_card_be_ca_url" {
  name                = "ced-card-be-ca-url"
  api_management_name = azurerm_api_management_api.ced_card_be_v1.api_management_name
  resource_group_name = azurerm_api_management_api.ced_card_be_v1.resource_group_name
  display_name        = "ced-card-be-ca-url"
  secret              = true
