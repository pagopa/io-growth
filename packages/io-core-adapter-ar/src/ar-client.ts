...
export const createArClient = (config: ArClientConfig): ArClient => ({
  documentContentClient: createDocumentContentClient(config),
  institutionClient: createInstitutionClient(config),
  onboardingClient: createOnboardingClient(config),
  userClient: createUserClient(config),
});
