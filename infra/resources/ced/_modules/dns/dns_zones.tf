...
resource "azurerm_dns_cname_record" "dkim1" {
  name                = "trwtzdool6n4argcfpo3pz3bvzpstaej._domainkey.ced.pagopa.it"
...
resource "azurerm_dns_cname_record" "dkim2" {
  name                = "u7rskmxwm6bpsh7ukjlonmacce35ygv5._domainkey.ced.pagopa.it"
...
resource "azurerm_dns_cname_record" "dkim3" {
  name                = "2oekew7jzmtnpendmmesp6jv37ptf2ak._domainkey.ced.pagopa.it"
...
resource "azurerm_dns_mx_record" "cmf_mx_2" {
  name                = "bounce.euc1.ced.pagopa.it"
  zone_name           = azurerm_dns_zone.public.name
  resource_group_name = var.resource_group_name
  ttl                 = 300
